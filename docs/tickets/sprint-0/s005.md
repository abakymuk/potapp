...
S-00.5.e — Smoke-тест 1. Создай фичфлаг new_checkout_flow в PostHog (release globally / 50%). 2. Локально: pnpm dev → открой /ff-demo, убедись, что UI меняется при переключении флага в PostHog (возможно нужен reload). 3. Worker: wrangler dev → GET /ff-demo?distinct_id=tester возвращает JSON с enabled: true|false. 4. Отключи интернет/подмени host → убедись, что вернётся enabled:false с source:'fallback'.
